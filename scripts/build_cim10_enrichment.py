...
consumed at runtime by the refacto pipeline (via scripts/prepare_referentials.py).
...
from rdflib import OWL, RDF, RDFS, Graph, Namespace  # type: ignore[import-untyped]
from rdflib.namespace import DC, SKOS  # type: ignore[import-untyped]
...
def validate_hierarchy(
    df: pd.DataFrame, expected_count: int = 19075, tolerance: int = 500
) -> list[str]:
    ...
    if abs(count - expected_count) > tolerance:
        warnings.append(
            f"Unexpected concept count: {count} (expected ~{expected_count} ± {tolerance})"
        )
...
def main() -> int:
    ...
    logger.info("Parsing {} ...", source_path)
    try:
        ...
        logger.error("RDF parsing failed: {}", exc)
        return 2
    ...
    logger.info("Wrote {} hierarchy rows to {}", len(hierarchy_df), hierarchy_path)
    logger.info("Wrote {} notes rows to {}", len(notes_df), notes_path)
    ...
    logger.info("Codes with inclusion notes: {}", inclusion_count)
    logger.info("Codes with exclusion notes: {}", exclusion_count)
